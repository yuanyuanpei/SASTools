...
data xxx2; 
   *keep pname; 
   length pname   $60 FileRef $8; 
    /* Assign the fileref */ 
   call missing(FileRef); /* Blank, so SAS will assign a file name */ 
   rc1 = filename(FileRef, "&root.\&_mode.\Data\&_Ldate."); /* Associate the file name with the directory */ 
   if rc1 ^= 0 then 
      abort; 
...
data file2;
set xxx2;
if index(pname,'2021_760_00CH1')>0; 
run;
proc sql noprint;
select pname into: fnameold from file2;
quit;
/*%put "&fnameold.";*/
/*%put "&root.\&_mode.\Data\&_Ldate.\&fnameold.";*/
...
data xxx; 
   *keep pname; 
   length pname   $60 FileRef $8; 
    /* Assign the fileref */ 
   call missing(FileRef); /* Blank, so SAS will assign a file name */ 
   rc1 = filename(FileRef, "&root.\&_mode.\Data\&date."); /* Associate the file name with the directory */ 
   if rc1 ^= 0 then 
      abort; 
    /* Open the directory for access by SAS */ 
   DirectoryID = dopen(FileRef); 
...
data file1;
set xxx;
if index(pname,'Tracking')>0 then dname="StatusTracking";
else if index(pname,'user_list')>0 then dname="UserList";
else if index(pname,'BO4.2')>0 then dname="PGSDV";
else if index(pname,'2021_760_00CH1')>0 then dname="EDC";
else dname=scan(scan(pname,2,'-'),1,' ');
run;
proc sql noprint;
select pname,dname,count(pname) into
:fnames separated by "#",
:dnames separated by "#",
:count
from file1;
